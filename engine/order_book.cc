...
void OrderBook::add_order(const Order& order) {
    ...
    if (it == id_map.end()) {
        throw std::runtime_error("Order ID already exists: " + std::to_string(order.order_id));
    }
    if (order.side == 1) {
        buy_orders[order.price].push_back(std::make_unique<Order>(order));
        id_map[order.order_id] = {order.price, true, std::prev(this->buy_orders[order.price].end())};
    } else {
        sell_orders[order.price].push_back(std::make_unique<Order>(order));
        id_map[order.order_id] = {order.price, false, std::prev(this->sell_orders[order.price].end())};
    }
}
